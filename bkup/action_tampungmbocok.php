  ...
  <div class="col-md-12">
    <div class="form-group">
      ...
      <select class="form-control" name="uIDMesin">
        ...
        <?php
        $sql = "SELECT * from mesin order by idMesin asc";
        $result = mysqli_query($link, $sql);
...
$idKerupuk = $_POST['idKerupuk'];
?>
<div class="table-responsive" id="sort">
  <div class="form-group">
    <label class="col-md-4 control-label"> KERUPUK : </label>
    <?php
    $sql = "SELECT * from barang where idBarang=".$idKerupuk;
  ...
  $sqlData="SELECT *
      FROM prosesproduksi
      WHERE barang_idBarang =$idKerupuk";
  $res= mysqli_query($link,$sqlData); 
  $r_data=mysqli_fetch_array($res);
  if($r_data['urutan']!=0){ ?>
    <div class="btn-group">    
      <a href="#modalInfo" class="btn btn-primary" data-toggle="modal"><i class="fa fa-info-circle"></i></a>
        <span class="sr-only">Toggle Dropdown</span>
      <button type="submit" class="btn btn-primary">Simpan Urutan</button>
    </div>
    <span class="help-block" style="font-weight: bold">*Jangan lupa klik apabila urutan proses sudah benar</span>
  <?php } ?>
  </div>
</div>
...
<div class="form-group">
  ...
  <select name="uKerupuk" class="form-control">
    ...
    while($rowModal = mysqli_fetch_array($result)){
      $rowModal['jumlah'] = $rowModal['jumlah'] - $rowModal['jumlah_realisasi'];
